...
from apps.purchases.schema import PurchaseBaseSchema, PurchaseInputCreateSchema, PurchaseProductInputSchema
...
class PurchasesService:
    ...
    def get_purchase_products_by_purchase_id_and_product_id(self, purchase_id: uuid.UUID, product_id: uuid.UUID):
        return PurchaseProduct.objects.filter(purchase_id=purchase_id, product_id=product_id).first()
    
    def get_purchase_by_id(self, purchase_id: uuid.UUID):
        return Purchase.objects.filter(id=purchase_id).first()
    
    def delete_purchase_product_by_purchase_id_and_product_id(self, purchase_id: uuid.UUID, product_id: uuid.UUID):
        return self.get_purchase_products_by_purchase_id_and_product_id(purchase_id, product_id).delete()
    
    def update_purchase_product_by_purchase_id_and_product_id(self, purchase_id: uuid.UUID, product_id: uuid.UUID, input_data: PurchaseProductInputSchema):
        purchase_product = self.get_purchase_products_by_purchase_id_and_product_id(purchase_id, product_id)
    ...
    def create_purchase_product(self, purchase_id: uuid.UUID, input_data: PurchaseProductInputSchema):
        return PurchaseProduct.objects.create(purchase_id=purchase_id, **input_data.dict())
    
    def create_purchase_products_in_list(self, purchase_id: uuid.UUID, list_purchase_product: list[PurchaseProductInputSchema]):
